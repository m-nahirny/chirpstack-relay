...
bool CayenneLPPDec::ParseLPP(const uint8_t *pBuffer, size_t Len, std::string *json_output)
{
	json root;
	while (Len > 2)
	{
		...
		if (lpp_type == LPP_DIGITAL_INPUT) {
			...
			root["type"] = "digital_input";

			root["value"] = pBuffer[2];
			...
			root["channel"] = channel;
			root["type"] = "analog_input";

//			float value = float((pBuffer[2] << 8) | pBuffer[3]) / 100.0f;
			float value = float(pBuffer[2]*256 + pBuffer[3]) / 100.0f;
			root["value"] = value;
...
//			float value = float((pBuffer[2] << 8) | pBuffer[3]) / 100.0f;
			...
			root["channel"] = channel;
			root["type"] = "luminosity";

//			uint16_t value = (pBuffer[2] << 8) | pBuffer[3];
			uint16_t value = pBuffer[2]*256 +  pBuffer[3];
			root["value"] = value;
			...
			if (Len < LPP_TEMPERATURE_SIZE)
				return false;
puts("Decoding temperature data");
			root["channel"] = channel;
			root["type"] = "temp";

//			float value = float(int16_t((pBuffer[2] << 8) | pBuffer[3])) / 10.0f;
			float value = float(int16_t(pBuffer[2]*256 + pBuffer[3])) / 10.0f;
...
//			uint32_t value = (pBuffer[2] << 24) | (pBuffer[3] << 16) | (pBuffer[4] << 8) | pBuffer[5];
			uint32_t value = (pBuffer[2]*256*256*256 + pBuffer[3]*256*256 + pBuffer[4] << 8*256 + pBuffer[5];
			root["value"] = value;
